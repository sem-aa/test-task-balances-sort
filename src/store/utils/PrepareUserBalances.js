const formatNumCurrency = (input, decimals) =>
  (Number(input) / 10 ** 8).toFixed(decimals || 5);
...
export function prepareUserBalances(dispatch, data, getState) {
    ...
    for (const key in data.list) {
      ...
      if (!tickers_balances[iso3].hasOwnProperty(type)) {
        tickers_balances[iso3][type] = {
          ...
          decimal:
            currency_data[coinId]?.decimal_rounding ?? currency_decimals[iso3],
          decimal_rounding: currency_data[usdtId].decimal_rounding,

          total_balance: formatNumCurrency(
            item.balance,
            currency_decimals[iso3]
          ),
          total_balance_usdt:
            item.balances && "USDT_total" in item.balances
              ? formatNumCurrency(
                  item.balances.USDT_total,
                  currency_data[usdtId].decimal_rounding
                )
              : null,
          total_balance_num: +item.balance,
          total_balance_usdt_num:
            item.balances &&
            "USDT_total" in item.balances &&
            +item.balances.USDT_total,

          total_available: formatNumCurrency(
            item.balance_available,
            currency_decimals[iso3]
          ),
          total_available_usdt:
            item.balances && "USDT" in item.balances
              ? formatNumCurrency(
                  item.balances.USDT,
                  currency_data[usdtId].decimal_rounding
                )
          ...
          frozen: formatNumCurrency(
            item.balance - item.balance_available,
            currency_decimals[iso3]
          ),
          frozen_usdt:
            item.balances && "USDT" in item.balances
              ? formatNumCurrency(
                  item.balances.USDT_total - item.balances.USDT,
                  currency_data[usdtId].decimal_rounding
                )
              : null,
        };
      ...
      if (total_balances[type] !== undefined) {
        total_balances[type] = {
          btc:
            total_balances[type].btc +
            (item.balances.BTC === undefined ? 0 : Number(item.balances.BTC)),
          usdt:
            total_balances[type].usdt +
            (item.balances.USDT === undefined ? 0 : Number(item.balances.USDT)),
        };
      }
    }
  }
  ...
  for (let key in total_balances) {
    total_balances[key].btc = formatNumCurrency(
      total_balances[key].btc,
      currency_data[btcId].decimal_rounding
    );
    total_balances[key].usdt = formatNumCurrency(
      total_balances[key].usdt,
      currency_data[usdtId].decimal_rounding
    );
  }
  console.log("tickers_balances", tickers_balances);
